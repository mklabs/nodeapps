...
* @author mdaniel
*/
 (function($, W, H, L, undefined) {
   

   var historyAble = (Modernizr && Modernizr.history) || (H && H.pushState);
  ...
  $.widget('ui.treeslider', {
    ...
    _create: function(o) {
      ...
      slider
        .delegate('a.js-slide-to, .breadcrumb a', 'click', function clickHandler(e) {
          ...
          if (self.sliding) {
            ...
            loading.filter(':visible').removeClass(o.frames.loading)
          }
        });
    ...
    pathFromURL: function(path) {
      var r = new RegExp('/(tree|blob)/');
      path = path.split(r)[2] || '/';
      if (path.slice(path.length - 1, path.length) != '/') path += '/';
      return path;
    },
    ...
    slideTo: function(url) {
      ...
      if(!frame.is('.' + o.frames.center) && current !== '/') {
    ...
    slideForwardTo: function(url) {
      ...
      if (!frame.length) {
        ...
        this.loadFrame(url, function(resp) {
          loading.replaceWith($(resp).find('.' + o.frames.center));
        })
      } else {
...
})(this.jQuery, this, this.history, this.location);